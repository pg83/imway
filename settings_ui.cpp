...
namespace {
    ...
    void settingText(const char* id, Settings& settings, StringView value, TextSetter setter) {
        Buffer text(value);
        char* data = text.cStr();

        if (ImGui::InputText(id, data, text.capacity(), ImGuiInputTextFlags_CallbackResize, resizeText, &text)) {
            (settings.*setter)(StringView((const char*)text.data()));
        }
    }

    void settingTextMultiline(const char* id, Settings& settings, StringView value, float height, TextSetter setter) {
        Buffer text(value);
        char* data = text.cStr();

        if (ImGui::InputTextMultiline(id, data, text.capacity(), ImVec2(-FLT_MIN, height), ImGuiInputTextFlags_CallbackResize, resizeText, &text)) {
            (settings.*setter)(StringView((const char*)text.data()));
        }
    }

    void settingCheckbox(const char* id, Settings& settings, bool value, BoolSetter setter) {
        if (ImGui::Checkbox(id, &value)) {
            (settings.*setter)(value);
        }
    }

    void settingSliderFloat(const char* id, Settings& settings, float value, float low, float high, const char* format, FloatSetter setter) {
        if (ImGui::SliderFloat(id, &value, low, high, format, ImGuiSliderFlags_AlwaysClamp)) {
            (settings.*setter)(value);
        }
    }

    void settingSliderInt(const char* id, Settings& settings, int value, int low, int high, const char* format, IntSetter setter) {
    ...
    void settingCombo(const char* id, Settings& settings, T value, const char* const (&items)[N], void (Settings::*setter)(T)) {
        int index = (int)value;
    ...
    void settingTime(const char* id, Settings& settings, int minute, IntSetter setter) {
        char value[16];
...
void Dialog::pageDisplay(Composer& c, Settings& s) {
    ...
    row("brightness step");
    settingSliderFloat("##brightness-step", s, s.brightnessStep(), .01f, .25f, "%.2f", &Settings::setBrightnessStep);
    row("hdr");
    settingCheckbox("enabled##hdr", s, s.hdrEnabled(), &Settings::setHdrEnabled);

    if (s.hdrEnabled()) {
        row("sdr white");
        settingSliderFloat("##sdr-white", s, s.sdrNits(), 80.f, 300.f, "%.0f nits", &Settings::setSdrNits);
        row("hdr key step");
        settingSliderFloat("##hdr-step", s, s.hdrStepNits(), 1.f, 50.f, "%.0f nits", &Settings::setHdrStepNits);
        row("display minimum");
        settingSliderFloat("##display-min", s, s.displayMinNits(), 0.f, 10.f, "%.3f nits", &Settings::setDisplayMinNits);
        row("display peak");
        settingSliderFloat("##display-peak", s, s.displayPeakNits(), 100.f, 10000.f, "%.0f nits", &Settings::setDisplayPeakNits);
        row("display maxFALL");
        settingSliderFloat("##display-fall", s, s.displayMaxFallNits(), 100.f, 10000.f, "%.0f nits", &Settings::setDisplayMaxFallNits);
    }
...

    {
        ...
        int value = s.outputBpc() == 8 ? 1 : s.outputBpc() == 10 ? 2 : s.outputBpc() == 12 ? 3 : 0;
...

    {
        ...
        settingCombo("##range", s, s.outputRange(), items, &Settings::setOutputRange);
    }
    ...
    settingCheckbox("##lock-before-dpms", s, s.lockBeforeDpms(), &Settings::setLockBeforeDpms);
    row("osd duration");
    settingSliderFloat("##osd", s, s.osdSeconds(), .25f, 5.f, "%.2f sec", &Settings::setOsdSeconds);
    row("osd fade");
    settingSliderFloat("##osd-fade", s, s.osdFadeSeconds(), .05f, 2.f, "%.2f sec", &Settings::setOsdFadeSeconds);
    ImGui::EndTable();
}
...
void Dialog::pageInput(Composer& c, Settings& s) {
    ...
    row("pointer speed");
    settingSliderFloat("##pointer-speed", s, s.pointerSpeed(), -1.f, 1.f, "%.2f", &Settings::setPointerSpeed);
    row("acceleration");
...

    {
        ...
        settingCombo("##click-method", s, s.touchpadClickMethod(), items, &Settings::setTouchpadClickMethod);
    }
...

    {
        ...
        settingCombo("##scroll-method", s, s.touchpadScrollMethod(), items, &Settings::setTouchpadScrollMethod);
    }
    ...
    row("swipe left");
    settingCombo("##swipe-left", s, s.swipeLeft(), gestureItems, &Settings::setSwipeLeft);
    row("swipe right");
    settingCombo("##swipe-right", s, s.swipeRight(), gestureItems, &Settings::setSwipeRight);
    row("swipe up");
    settingCombo("##swipe-up", s, s.swipeUp(), gestureItems, &Settings::setSwipeUp);
    row("swipe down");
    settingCombo("##swipe-down", s, s.swipeDown(), gestureItems, &Settings::setSwipeDown);
    row("pinch in");
    settingCombo("##pinch-in", s, s.pinchIn(), gestureItems, &Settings::setPinchIn);
    row("pinch out");
    settingCombo("##pinch-out", s, s.pinchOut(), gestureItems, &Settings::setPinchOut);
    ImGui::EndTable();

    size_t devices = s.inputDeviceCount();

    for (size_t i = 0; i < devices && i < Settings::inputDeviceCapacity; i++) {
...
void Dialog::pageShortcuts(Composer&, Settings& s) {
    ...
    for (size_t i = 0; i < Settings::shortcutCount; i++) {
        ...
        const char* label = *shortcutCapture == (int)i ? "press a key..." : chord;
...
void Dialog::pageNotifications(Composer&, Settings& s) {
    ...
    row("dnd schedule");
    settingCheckbox("##dnd-schedule", s, s.dndScheduled(), &Settings::setDndScheduled);

    if (s.dndScheduled()) {
        row("starts");
        settingTime("##dnd-start", s, s.dndStartMinute(), &Settings::setDndStartMinute);
        row("ends");
        settingTime("##dnd-end", s, s.dndEndMinute(), &Settings::setDndEndMinute);
    }

    row("default timeout");
    settingSliderFloat("##notification-timeout", s, s.notificationSeconds(), 1.f, 30.f, "%.1f sec", &Settings::setNotificationSeconds);
    row("history limit");
    settingSliderInt("##notification-history", s, s.notificationHistory(), 0, 1000, "%d", &Settings::setNotificationHistory);
    row("toast width");
    settingSliderFloat("##notification-width", s, s.notificationWidth(), 200.f, 800.f, "%.0f px", &Settings::setNotificationWidth);
    row("toast position");

    {
        ...
        settingCombo("##toast-position", s, s.toastPosition(), items, &Settings::setToastPosition);
    }

    row("wifi events");
    settingCheckbox("##notify-wifi", s, s.notifyWifi(), &Settings::setNotifyWifi);
    row("critical notifications");
    settingCheckbox("##critical", s, s.allowCriticalNotifications(), &Settings::setAllowCriticalNotifications);
    ImGui::EndTable();

    ImGui::SeparatorText("application rules");
    size_t count = s.notificationRuleCount();

    for (size_t i = 0; i < count && i < Settings::notificationRuleCapacity; i++) {
    ...
    if (count < Settings::notificationRuleCapacity && ImGui::SmallButton("add application rule")) {
...
void Dialog::draw(Composer& c, Settings& s, int& capture, bool& open) {
    ...
    for (int i = 0; i < (int)(sizeof(generatedSettingsPages) / sizeof(*generatedSettingsPages)); i++) {
...
void drawSettings(Composer& c, Settings& settings, bool toggle, int& shortcutCapture, DialogState** state) {
