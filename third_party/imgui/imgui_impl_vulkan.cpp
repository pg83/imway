...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer, ImGui_ImplVulkan_FrameRenderBuffers* rb, int fb_width, int fb_height)
{
    ...
    // Our visible imgui space lies from draw_data->DisplayPps (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayPos is (0,0) for single viewport apps.
    {
        ...
        struct { int primaries; float reference_nits; float min_nits; float max_nits; } color = {0, 0.0f, 0.0f, 0.0f};
...
void ImGui_ImplVulkan_SetTextureColor(int source, int primaries, float reference_nits, float min_nits, float max_nits)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_RenderState* state = (ImGui_ImplVulkan_RenderState*)ImGui::GetPlatformIO().Renderer_RenderState;
    if (!bd || !state)
        return;
    struct { int primaries; float reference_nits; float min_nits; float max_nits; } color = {primaries, reference_nits, min_nits, max_nits};
...
{
    ImGui_ImplVulkan_SetTextureColor((int)(intptr_t)cmd->UserCallbackData, 0, 0.0f, 0.0f, 0.0f);
}
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    if (!bd->PipelineLayout)
    {
        ...
        push_constants[0].size = sizeof(float) * 10;
