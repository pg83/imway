// A source-less drag stays inside the client that started it: over another
// client's window, and over no window at all, the dragging client's own
// surface is left and nothing is entered; back over it, it is entered
...
int main(void) {
    ...
    wl_make_toplevel(&top, "dnd-null-source-foreign", 400, 300, 0xFFFF0000);
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        if (dropped) {
            if (entered != 3 || left != 2) {
                fprintf(stderr, "drop after %d enters and %d leaves, want 3 and 2\n", entered, left);
                return 1;
            }
