...
"$IMWAY_TESTS_BIN/client_reg_input_tablet_tools_uinput" "$XDG_RUNTIME_DIR/input" "$px" "$py" \
    >"$PEN_LOG" 2>&1 &
PEN_PID=$!
...
touch go-tools
await 200 grep -q "tools done" "$PEN_LOG" || { echo "the tools never moved"; cat "$PEN_LOG"; exit 1; }

wait_client "prox_in"
wait_client "tablet: slider"
wait_client "tablet: rotation"
wait_client "tablet: wheel"
...
expect_alive "compositor died carrying tablet tools through libinput"
echo "OK: libinput carries the airbrush slider and the puck rotation and wheel into tablet-v2"
