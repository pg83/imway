...
await 50 test "$(probes event5)" -gt "$before5" || { echo "the plugged event5 was never tried"; cat "$IMWAY_LOG"; exit 1; }

# the watch keeps working: a second try at the same slot is made
rm "$dir/event5"
touch "$dir/event5"
await 50 test "$(probes event5)" -gt "$((before5 + 1))" || { echo "the slot was taken by a file that is no device"; cat "$IMWAY_LOG"; exit 1; }
