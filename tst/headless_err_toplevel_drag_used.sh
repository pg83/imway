#!/usr/bin/env bash
# Asking for an xdg_toplevel_drag on a source already dragging is
# INVALID_SOURCE; the compositor ends the drag with the client and lives on.
set -euo pipefail
. "$(dirname "$0")/lib.sh"
...
echo "OK: a dragging source gets no toplevel drag object"
