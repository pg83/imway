# Six sealed wl_shm buffers laid out as a zero-copy import cannot take them
# (client_reg_shm_layouts: odd stride, odd offset, an offset off the page,
# a pool that is not whole pages, two buffers sharing a pool), on the
# backend the scenario forces: each falls back to the CPU copy or to what
# the device can do, and each is drawn with its rows where they belong,
# its colour in the top half and white in the bottom one.
...
tops = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (0, 255, 255), (255, 0, 255)]
bad = []
