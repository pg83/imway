...
# A seat manager (the stand-in seatd of headless_kms_seatd) that answers a
# request the compositor never made: libseat cannot follow the conversation
# any more, so the seat is as good as lost and the compositor exits.
set -euo pipefail
. "$(dirname "$0")/lib.sh"
...
echo garbage > "$XDG_RUNTIME_DIR/seatd-ctl"
await 100 in_log "seat connection lost, exiting" || { echo "a garbled seat conversation went unnoticed"; cat "$IMWAY_LOG"; exit 1; }
await 100 in_log "clean exit" || { echo "the compositor outlived its seat"; exit 1; }
echo "OK: a seat manager the compositor cannot follow ends the session"
