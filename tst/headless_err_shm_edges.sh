...
# no height, rows past a 32-bit size, larger than the pool, past its end)
# are invalid_stride, and a pool resized to nothing is invalid_fd.
set -euo pipefail
. "$(dirname "$0")/lib.sh"

for mode in negative-offset no-width no-height rows-overflow past-pool past-end resize-zero; do
