// Regression: --scale changes compositor chrome only. It must not become a
// fractional output scale while wl_output and xdg-output still expose a 1:1
// logical coordinate space, or clients allocate needlessly huge buffers.
...
int main(void) {
    ...
    if (!mgr) {
        fprintf(stderr, "no fractional-scale manager\n");
        return 1;
    }
    ...
    if (!got_scale) {
        fprintf(stderr, "no preferred_scale\n");
        return 1;
    }
    printf("preferred_scale=%u\n", scale);
    if (scale != 120) {
        fprintf(stderr, "ui scale leaked into preferred_scale: %u/120\n", scale);
        return 1;
    }
    return 0;
}
