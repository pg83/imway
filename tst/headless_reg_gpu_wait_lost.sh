...
# without: the readback of the final frame that --screenshot saves. The
...
rc=0
out=$(env IMWAY_CHAOS=gpu-wait=0 timeout 60 "$imway_bin" --device headless --socket imway-wait --frames 3 --screenshot "$shot" 2>&1) || rc=$?
