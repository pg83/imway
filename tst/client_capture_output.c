...
// An ext-image-copy-capture of the output that maps no window of its own:
// whatever is on screen is captured into an shm buffer and written to the
// PPM file argv[1]. For scenarios where another client owns the screen (a
// fullscreen window on the scanout plane).
//   usage: client_capture_output OUT.ppm
...
int main(int argc, char** argv) {
    ...
    int fd = memfd_create("capture-output", 0);
