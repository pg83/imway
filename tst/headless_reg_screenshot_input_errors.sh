...
# shared-buffer metadata it cannot parse (too few fields, a field past 64
# bits, a GPU id that is not hex, a zero stride), or whose buffer cannot be
# opened, each open the 480x180 error panel, which Escape dismisses with
# status 0. Well-formed metadata naming a GPU no device has gets no window:
# the viewer says so and exits 1. Colour metadata it only partly
# understands falls back to SDR and still shows the image.
set -euo pipefail
. "$(dirname "$0")/lib.sh"
