...
# CloseNotification and an unknown method are answered, and an expired
# notification is closed only once.
set -euo pipefail
. "$(dirname "$0")/lib.sh"
...
wait_client "expired notification closed once"
wait_client "notifications malformed done"
expect_client_ok "the malformed notification client failed"

# the four odd-hint notifications and the brief one, and none of the
# refused ones
[[ "$(dump_field '^notifications' history)" == 5 ]] || { echo "the refused calls left notifications behind"; dump_state; exit 1; }
