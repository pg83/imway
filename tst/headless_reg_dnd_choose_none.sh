...
# compositor's pick of what both sides offer; nothing is shared: no action, and the drop cancels the source.
set -euo pipefail
. "$(dirname "$0")/lib.sh"
...
echo "OK: nothing is shared: no action, and the drop cancels the source"
