...
# leaves when dismissed. Two ways to get there: a filename template that
# expands past the name limit, and a directory that cannot be created
# because a regular file sits where it should be.
set -euo pipefail
. "$(dirname "$0")/lib.sh"
