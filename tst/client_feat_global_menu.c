...
static void dict_icon_data(DBusMessageIter* dict) {
    // A 1x1 opaque red RGBA PNG, transported as the DBusMenu icon-data byte
    // array. The IDAT checksum has to be right: libpng reads the header
    // happily and only rejects the image when the pixels are pulled, which
    // is quiet enough that a wrong one went unnoticed here for a long time.
