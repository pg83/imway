#!/usr/bin/env bash
# The window alt-tab selected goes away while Alt is held. Unmapped, it is
# not raised when Alt comes up, and the focus stays where it was. Destroyed,
# the next Tab starts from nothing selected and finds no mapped window, and
# Alt coming up commits nothing.
set -euo pipefail
. "$(dirname "$0")/lib.sh"
...
ctl "key 15 press"; ctl "key 15 release" # KEY_TAB: selects a
...
ctl "key 56 press"
ctl "key 15 press"; ctl "key 15 release" # b, the lone mapped window
