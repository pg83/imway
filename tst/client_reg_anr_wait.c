// A server-side-decorated toplevel that maps and then hangs: no dispatch,
// no pongs, until SIGUSR1 wakes it; from then on it services the
// connection again and answers the pings it missed.
#include "wl_util.h"
...
int main(void) {
    ...
    xdg_toplevel_set_title(tl, "anr-wait-client");
    xdg_toplevel_set_app_id(tl, "anr-wait-client");
