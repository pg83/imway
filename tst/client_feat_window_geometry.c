// Feature: xdg_surface.set_window_geometry crops the visible window to the
// geometry rect: a 300x200 buffer with a 20px magenta margin and an orange
// 260x160 core, geometry (20,20,260,160) — no magenta may reach the screen.
...
static struct wl_buffer* margin_buffer(void) {
    int w = 300, h = 200, stride = w * 4, size = stride * h;
    int fd = memfd_create("geom-shm", 0);
    if (fd < 0 || ftruncate(fd, size) < 0) { perror("memfd"); exit(1); }
    uint32_t* px = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    for (int y = 0; y < h; y++)
        for (int x = 0; x < w; x++) {
            int core = x >= 20 && x < 280 && y >= 20 && y < 180;
            px[y * w + x] = core ? 0xFFFF8000 : 0xFFFF00FF;
        }
    munmap(px, size);
...
static void xs_configure(void* d, struct xdg_surface* s, uint32_t serial) {
    ...
    if (!committed) {
        xdg_surface_set_window_geometry(xs, 20, 20, 260, 160);
        wl_surface_attach(surface, margin_buffer(), 0, 0);
...
int main(void) {
    ...
    xdg_toplevel_set_app_id(tl, "geometry");
    wl_surface_commit(surface);
