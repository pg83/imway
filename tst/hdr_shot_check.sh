# Checks for a saved screenshot against two captures of the output taken
# around the save (only pixels equal in both count, the rest of the desktop
# may have moved on): an SDR session's PNG, or an HDR (BT.2100 PQ) one's.
...
# an SDR session's PNG is the frame itself: every stable pixel equals the
# output's capture within <tolerance> codes
sdr_png_check() { # <png> <before.ppm> <after.ppm> <tolerance>
    python3 - "$@" <<'PY'
import struct
import sys
import zlib
...
for y in range(0, h, 7):
    for x in range(0, w, 7):
        ...
        if before[i:i + 3] != after[i:i + 3]:
            continue
        checked += 1
        if all(abs(png[y][x * 4 + c] - before[i + c]) <= tolerance for c in range(3)):
            same += 1
print(f"sdr stable samples={checked} same={same}")
assert checked >= 1000 and same >= checked * .95, "the PNG is not the output's frame"
PY
}
