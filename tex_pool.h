...
// imgui's texture layout (one combined image sampler at binding 0, fragment
// stage), so the sets it returns bind to imgui's pipeline as ImTextureID.
//
...
struct VkTexturePool {
    ...
    virtual VkDescriptorSet alloc(VkImageView view, VkImageLayout layout, VkDescriptorPool& outPool) = 0;
