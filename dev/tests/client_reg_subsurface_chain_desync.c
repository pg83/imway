...
static int run_chain(struct wl_toplevel_ctx* top, int depth, int destroy_topdown) {
    for (int i = 0; i < depth; i++) {
        ...
        wl_subsurface_set_desync(chain_sub[i]);
    }
...
int main(void) {
    ...
    wl_make_toplevel(&top, "chain-desync", 64, 64, 0xffff0000);

    if (run_chain(&top, 64, 0)) return 1;
    if (run_chain(&top, 256, 1)) return 1;
    if (run_chain(&top, 1024, 0)) return 1;
    return 0;
}
