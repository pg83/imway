// Regression: cursor-shape-v1 fidelity. Every shape the protocol names must
// reach the scene verbatim; collapsing shapes onto a lookalike loses the
// client's intent. The scenario acks each observed shape with a pointer
// wiggle, whose motion event releases the next set_shape.
...
int main(void) {
    ...
    wl_make_toplevel(&top, "client_reg_cursor_shape_full", 400, 300, 0xFFFF0000);
    printf("client_reg_cursor_shape_full: mapped\n");

    // wait for pointer focus (the scenario points at us)
    ...
    for (uint32_t shape = WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_DEFAULT;
         shape <= WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_ALL_RESIZE; shape++) {
        int seen = wlp_motion_count;
        wp_cursor_shape_device_v1_set_shape(shape_dev, wlp_enter_serial, shape);
        ...
        if (wlp_motion_count == seen) { fprintf(stderr, "no ack for shape %u\n", shape); return 1; }
    }

    if (wl_display_roundtrip(wl_dpy) < 0) { fprintf(stderr, "display error\n"); return 1; }
    printf("client_reg_cursor_shape_full: ok\n");
    return 0;
}
