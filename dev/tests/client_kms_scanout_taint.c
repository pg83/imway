/* A fullscreen 1280x800 XRGB dmabuf toplevel — the perfect direct-scanout
 * candidate. The buffer is a DRM dumb buffer from a card node, filled solid
 * red, so the scenario can trace it through composition after the fake KMS
 * rejects its flip and the buffer gets tainted. */
#include "wl_util.h"
