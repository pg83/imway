...
// the inspector is a plain imgui widget, not an entity. its state is a
// self-owned opaque handle in the caller's slot: nullptr = closed. *state
// is also the caller's "is it open" answer; closing retires the handle's
// entire ObjPool. the window's own close button ends it too. imgui calls inside, so
