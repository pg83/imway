...
// field, then the xdg desktop entries; up/down + enter or a mouse click.
// imgui calls inside, so this runs between NewFrame and Render. returns
// true when the user picked something; the command line is appended to run,
// which is caller memory precisely because the dialog dies with the pick
bool drawLauncher(int screenW, int screenH, float uiScale, IconStore& icons, IconResolver& texes, bool toggle, void** state, stl::Buffer& run);
