...
namespace {
    ...
    // the picker's state line, in WifiState order
    constexpr const char* kStateLabels[] = {"no adapter", "disconnected", "connecting", "connected"};

    static_assert(sizeof(kStateLabels) / sizeof(kStateLabels[0]) == (size_t)WifiState::connected + 1, "the label table tracks WifiState");

    const char* stateLabel(WifiState s) {
        return kStateLabels[(int)s];
    }
}
...
void Dialog::draw(Composer& c, bool& open) {
    ...
    checkTrue(ImGui::Begin("##wifi", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoDocking));
    if (fresh) {
